...
import { setCurrentColorScheme, type ColorMode, type ColorScheme } from '@/utils/color-mode'
...
const labels = {
  system: 'Use light color mode',
  light: 'Use dark color mode',
  dark: 'Use system color mode'
}

const ThemeButton: Component = () => {
  const [isMounted, setMounted] = createSignal(false)
  const [colorMode, setColorMode] = createSignal<ColorMode>('system')

  function handleStorageChange() {
    const savedColorMode = localStorage.getItem(constants.colorMode.storageKey)
  ...
  function handleToggleTheme() {
    setColorMode((value) => {
      ...
      localStorage.setItem(constants.colorMode.storageKey, newValue)

      if (newValue === 'system') {
        const currentSystemColorScheme: ColorScheme = window.matchMedia(constants.colorMode.systemColorSchemeQuery)
          .matches
          ? 'dark'
          : 'light'
  ...
  return (
    <Show when={isMounted()} fallback={<span class={classnames(styles.size, styles.placeHolder)} />}>
      <button
